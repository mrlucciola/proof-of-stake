...
impl From<TxnSignatureSecpEcdsa> for secp256k1::ecdsa::Signature {
    fn from(x: TxnSignatureSecpEcdsa) -> secp256k1::ecdsa::Signature {
        secp256k1::ecdsa::Signature::from(x)
    }
}
...
impl TxnSignatureSecpEcdsa {
    pub fn msg_from_id(txn_id: &TxnId) -> Message {
        secp256k1::Message::from_slice(txn_id).unwrap()
    }
